import { ApplicationContest, CONTEST_STATUS } from '@prisma/client';
...
export class ApplicationContestDto extends BaseApplicationContestDto {
  ...
  @IsNotEmpty({ message: 'поле статуса заявки не может быть пустым' })
  status: string;
  ...
  constructor(applicationContest: ApplicationContestExtendedContestTeam) {
    ...
    this.status = applicationContest.status;
  }
}
